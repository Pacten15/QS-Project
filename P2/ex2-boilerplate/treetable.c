 ...
 *
 * @param[in] node the node whose height is being returned
 *
 * @return height of the node
 */
static int height(RBNode *node)
{
    if (node == NULL)
        return -1;
...
/**
 * Returns 1 if the tree table t is key balanced and 0 otherwise.
 *
 * @param[in] t the tree table being checked for key balance
 *
 * @return 1 if the tree table is key balanced and 0 otherwise
 */
...
/**
 * Returns 1 if the tree table t is key sorted and 0 otherwise.
 *
 * @param[in] t the tree table being checked for key sorting
 *
 * @return 1 if the tree table is key sorted and 0 otherwise
 */
int sorted(TreeTable* t)
{
    ...
    while (n != s) {
        if (n->left != s && tree_max(t, n->left)->key > n->key)
            return 0;

        if (n->right != s && tree_min(t, n->right)->key < n->key)
            return 0;
